...
export const StopTheScoreRound: React.FC<StopTheScoreProps> = ({ challenge, currentPlayer, onComplete }) => {
  ...
  const lockCurrentScore = () => {
    ...
    sound.playStamp(); setMeterValue(score); setLockedScore(score); setPhase('LOCKED');
  };
  ...
  const handleStartMeter = () => { sound.playClick(); setPhase('RUNNING'); startTimeRef.current = 0; reqRef.current = requestAnimationFrame(animateMeter); };
  const handleSelectOption = (idx: number) => { sound.playClick(); setSelectedOptionIndex(idx); const correct = idx === challenge.correctIndex; const score = correct ? (lockedScore ?? 0) : 0; setEarnedScore(score); setPhase('RESULT'); correct ? sound.playVictoryFanfare() : sound.playDisapproval(); };
  ...
  return (
    <div className="w-full max-w-5xl mx-auto font-['Plus_Jakarta_Sans']">
      <ApparatusFrame eyebrow="Confidence & Risk Chamber • Volatility Engine" title={challenge.prompt} subtitle={<><strong>{currentPlayer.name}</strong>, decide how many points your confidence deserves before reality is consulted.</>} icon={<Gauge size={29}/>} accent="#dd5e55" instrumentLabel="VOLATILITY GAUGE">
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_350px] gap-6 items-center">
          <div className="relative rounded-[30px] border-[5px] border-[#65442c] bg-[#e65d4e] p-5 shadow-[inset_0_0_0_5px_#f49d70,0_10px_0_#65442c]">
            ...
            <div className="rounded-[24px] border-[4px] border-[#65442c] bg-[#fff3cf] p-4 shadow-inner">
              ...
              <div className="mx-auto -mt-3 w-56 rounded-xl border-[4px] border-[#65442c] bg-[#253e46] p-3 text-center shadow-[0_5px_0_#65442c]">
                <div className="font-['Courier_Prime'] text-[9px] font-black uppercase tracking-widest text-[#9ee1dc]">{phase==='LOCKED'?'STAKE LOCKED':'LIVE SCORE'}</div>
                <div className="font-['Space_Mono'] text-5xl font-black text-[#f6d663]">{lockedScore ?? meterValue}</div>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-4 gap-2">{['PRESSURE','DOUBT','AUDACITY','REGRET'].map((label,i)=><div key={label} className={`rounded-lg border-2 border-[#65442c] py-2 text-center font-['Courier_Prime'] text-[8px] font-black ${i===2?'bg-[#f2cf65] text-[#65442c]':'bg-[#2f7f84] text-white'}`}>{label}</div>)}</div>
          </div>
          ...
          <div className="rounded-2xl border-[4px] border-[#65442c] bg-[#fff4d8] p-5 shadow-[0_7px_0_#65442c]">
            ...
            {phase==='LOCKED' && <div className="space-y-4"><div className="rounded-xl border-2 border-[#65442c] bg-[#f2cf65] p-3 text-center text-[#60452f]"><div className="font-['Courier_Prime'] text-[9px] font-black uppercase tracking-widest">Potential award</div><div className="font-['Space_Mono'] text-3xl font-black">{lockedScore}</div></div><div className="grid grid-cols-1 gap-2.5">{challenge.options.map((opt,idx)=><button key={idx} onClick={()=>handleSelectOption(idx)} className="flex items-center gap-3 rounded-xl border-[3px] border-[#65442c] bg-[#fdf8e8] p-3 text-left font-['Cinzel'] text-xs font-bold text-[#30434a] shadow-[0_3px_0_#65442c] hover:bg-[#d9efdf]"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#2fa8ae] font-['Space_Mono'] text-white">{String.fromCharCode(65+idx)}</span>{opt}</button>)}</div></div>}
          </div>
        </div>
      </ApparatusFrame>
    </div>
