...
export const Header: React.FC<HeaderProps> = ({ roundConfig, totalRounds = 6, players, currentPlayerIndex, onOpenAssets, onTriggerReview, canReview = false }) => {
    ...
    <header className="w-full flex flex-col gap-3 pb-3 mb-2">
      <div className="flex items-center justify-between gap-3 flex-wrap rounded-2xl border-[3px] border-[#7e5c24] bg-[#f5e7c3] px-4 py-2.5 shadow-[0_5px_0_#7b4f32,0_10px_20px_rgba(70,50,34,.14)]">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleToggleMute} className="bureau-button rounded-full bg-[#fff7df] p-2 text-[#244b55]" title={muted ? 'Unmute Bureau Audio' : 'Mute Bureau Audio'}>{muted ? <VolumeX size={16} /> : <Volume2 size={16} />}</button>
        </div>
      </div>

      {players.length > 0 && <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {players.map((p, idx) => { const isTurn = idx === currentPlayerIndex; return (
          <div key={p.id} className={`relative flex items-center justify-between gap-2 rounded-2xl border-[3px] px-3 py-2 transition-transform ${isTurn ? 'border-[#7e5c24] bg-[#2f8f95] text-[#fff7df] shadow-[0_5px_0_#7b4f32] -translate-y-0.5' : 'border-[#b58e59] bg-[#fff7df]/95 text-[#2f4248] shadow-[0_3px_0_#b18b60]'}`}>
            ...
            <div className="text-right shrink-0"><span className={`font-['Space_Mono'] font-black text-base ${isTurn ? 'text-white' : 'text-[#376d9b]'}`}>{p.score.toLocaleString()}</span><span className={`block font-['Courier_Prime'] text-[8px] uppercase ${isTurn ? 'text-[#d8f2ef]' : 'text-[#876a4d]'}`}>Pts</span></div>
          </div>
        ); })}
      </div>}
    </header>
