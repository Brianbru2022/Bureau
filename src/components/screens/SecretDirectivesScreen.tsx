...
export const SecretDirectivesScreen: React.FC<SecretDirectivesProps> = ({ players, onFinishDirectives }) => {
  const [currentPlayerIdx,setCurrentPlayerIdx]=useState(0); const [isRevealed,setIsRevealed]=useState(false); const [artFailed,setArtFailed]=useState(false); const [handoverConfirmed,setHandoverConfirmed]=useState(players.length===1);
  ...
  const handleNextPlayer=()=>{sound.playClick();setIsRevealed(false);setArtFailed(false);if(currentPlayerIdx+1<players.length){setCurrentPlayerIdx(v=>v+1);setHandoverConfirmed(false)}else onFinishDirectives();};

  if(players.length>1&&!handoverConfirmed)return <PrivacyCurtain recipient={player} purpose="A sealed Bureau directive is waiting. Its objective must remain unknown to every other candidate." confirmationLabel={`I am ${player.name} — receive directive`} onConfirm={()=>setHandoverConfirmed(true)}/>;
  ...
  return <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto py-6 px-4 font-['Plus_Jakarta_Sans']">
    ...
    <div className="w-full bureau-paper border-[4px] border-[#765139] rounded-[28px] p-6 shadow-2xl flex flex-col items-center gap-5 bureau-paper-drop">
      <div className="flex items-center gap-3 border-b-2 border-[#c7a675] w-full pb-3"><BureauAvatar player={player} size={54}/><div><span className="font-['Courier_Prime'] text-xs text-[#a9443d] uppercase font-black tracking-widest block">Confidential for eyes of</span><h3 className="font-['Cinzel'] font-black text-lg text-[#244b55]">{player.name}</h3></div></div>
      ...
      {!isRevealed&&<button onClick={handleNextPlayer} className="bureau-button w-full py-3 rounded-xl bg-[#e0a83f] text-[#49361e] font-['Cinzel'] font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2"><span>{currentPlayerIdx+1<players.length?`Prepare handover to ${players[currentPlayerIdx+1].name}`:'Commence Round 1'}</span><ArrowRight size={16}/></button>}
    </div>
  </div>;
};
